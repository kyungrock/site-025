...
HOME_STATIC = [
    {
        "href": "guide.html",
        "title": "고양이 마사지 가이드 — 입문·부위·안전",
        "summary": "스트로킹 기본부터 부위별 터치, 고양이 신호 읽기까지 단계별로 정리했습니다.",
        "category": "가이드",
        "date": "2026-06-11",
        "emoji": "📖",
    },
    {
        "href": "tips.html",
        "title": "고양이 마사지 실전 팁 — 저녁·브러시·노묘",
        "summary": "5분 루틴, 브러시 병행, 스트레스 신호 대응 등 상황별 실전 팁 모음.",
        "category": "팁",
        "date": "2026-06-14",
        "emoji": "💡",
    },
    {
        "href": "faq.html",
        "title": "고양이 마사지 FAQ — 자주 묻는 질문",
        "summary": "마사지 방법, 빈도, 싫어할 때, 금지 부위 등 자주 묻는 질문과 답변.",
        "category": "FAQ",
        "date": "2026-06-10",
        "emoji": "❓",
    },
...
def load_site() -> dict:
    ...
    site.setdefault("site_name", site.get("site_title", "고양이"))
    site.setdefault("tagline", f"{site.get('topic', '')} 관련 정보 허브")
    site.setdefault("domain", site.get("deploy_url", "https://site-025.b4a.app"))
    return site
...
def fb_header(site: dict, *, active: str = "", depth: int = 0) -> str:
    ...
    if depth == 0:
        ...
        <input type="search" placeholder="고양이 마사지 검색" aria-label="검색">
      </div>"""
...
def fb_footer(site: dict, depth: int = 0) -> str:
    p = "../" * depth
    host = esc(site.get("host_name", "Back4app"))
    slug = esc(site.get("slug", "site-025"))
...
def merge_draft(draft_path: Path, site: dict) -> dict:
    ...
    for p in new_posts:
        ...
        p.setdefault("author", site.get("site_name", "고양이"))
        p.setdefault("emoji", "🐱")
